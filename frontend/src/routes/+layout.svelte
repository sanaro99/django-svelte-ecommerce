    ...
    <div class="flex items-center space-x-4">
      ...
      {:else if user}
        ...
        <div class="relative">
          ...
          {#if dropdownOpen}
            <div class="absolute right-0 mt-2 w-48 bg-white shadow-lg rounded-md py-2 z-20">
      ...
      {:else}
        <button on:click={() => window.location.href = '/login'} class="px-3 py-2 rounded-md text-gray-700 hover:bg-gray-100 hover:text-indigo-600">Login</button>
        <button on:click={() => window.location.href = '/register'} class="px-3 py-2 rounded-md text-gray-700 hover:bg-gray-100 hover:text-indigo-600">Register</button>
      {/if}
    </div>
  </div>
</nav>
